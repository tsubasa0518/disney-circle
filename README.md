...
アフヌン同好会ページへ入るための簡易的なクライアントサイド入口ページです。

主な内容:

- テンキー形式のパスワード入力UI
- JavaScriptによる簡易的な入力チェック
- 誤入力時のロック表示
- 入力成功時のページ遷移

このページの処理は `js/afternoon-tea.js` で管理しています。

注意点:

- この仕組みは、GitHub Pages上で動くクライアントサイドの簡易ゲートです。
- サーバー側の本格的なアクセス制御ではありません。
- HTML、CSS、JavaScript、画像ファイルは静的ファイルとして公開される可能性があります。
- 個人情報、非公開写真、秘密情報、外部公開を前提としない内容は掲載しないでください。
